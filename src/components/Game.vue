  ...
  <div class="game-wrap" v-if="game">
    <div class="game-info-box">
      ...
      >
        ...
        <span
          v-on:click="timeSpell(game.gameId, participant.championId, participant.spell1Id)"
        >
          CD: {{participant.spell1cd}}
          <img :src="settings.static_base_url+participant.spell1Image" />||
        </span>
        <span v-on:click="timeSpell(game.gameId, participant.championId, participant.spell2Id)">
...
Vue.use(vueNativeSock, settings.websocket_url, { connectManually: true, format: "json" });
...
export default {
  ...
  methods: {
    timeSpell(gameId, championId, spellId) {
      const req = {
        action: "time",
        gameId: gameId,
        champKey: championId,
